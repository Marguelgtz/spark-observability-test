...
The committed `pnpm perf:query-plan` probe applies checked-in migrations to an in-memory SQLite database, seeds only synthetic rows (2 repositories, 100 PRs, 300 runs, 10 lifecycle rows), calls the real Dashboard/Overview reader functions, records their D1 statement boundaries, and emits `EXPLAIN QUERY PLAN`. The 2026-09-10 run produced 7 statements for the Dashboard reader composition, 3 for Overview evaluations, and 6 for merged-unresolved plus Outcome. The Activity plan materializes history and uses temporary B-trees for ordering/grouping; the Overview plan recomputes trend work alongside the page. A direct raw-timestamp and `datetime(...)` comparison both used a temporary sort B-tree on this schema, so no index rewrite is justified yet. These are reproducible local shape findings, not production latency or rows-read measurements.
