...
Status: **active living plan.** G0 through bounded G10 are **DONE** on the shadow `RepositoryUnderstanding` path. Spark retains exact-revision process/evidence facts, replays point-in-time state, derives bounded provenance-bearing process insights and historical measures, exposes them through a lifecycle-aware, abstention-safe agent context, and now models bounded deployment (CD) observations with separate approval state, provider-identified CI→deployment links, bounded deployment history, and shadow-path deployment webhook routing. CI/CD remains one neutral shadow input: no action, decision, or automatic steering exists. Unsupported expectations and incomplete absence remain `UNKNOWN`. CI-306 remains explicitly **BLOCKED but non-blocking**; CI-710 (deployment-state insight) unblocked with G10 and is **DONE**. Live ingestion and attention policy remain untouched. No CI-plan task is currently READY; the next work is a plan re-review (CI-306 alternatives or the next bounded extension).
...
| CI-710 | DONE | Deployment-state insight (waiting, running, failure, success). | G10 | G10 supplies deployment/approval observations; one `DEPLOYMENT_STATE` insight per deployment keeps waiting/queued/running/success/failure/cancelled/uninterpretable distinct, with provenance, completeness, and `SUPPORTED`/`TENTATIVE`/`UNKNOWN` confidence. |
...
| ID | Status | Task | Depends on | Acceptance evidence |
| --- | --- | --- | --- | --- |
| CI-1001 | DONE | Deployment observations (deployment, revision, environment, lifecycle, outcome, timestamps). | G2 | `DeploymentObservation` on the canonical `RepositoryUnderstanding` with the shared lifecycle/outcome vocabulary; exact-revision filtering; unknown provider states stay `UNKNOWN`. |
| CI-1002 | DONE | Waiting/approval state (approval gates distinguished from failures). | CI-1001 | Independent `approvalState` vocabulary (`APPROVED/WAITING/NOT_REQUIRED/UNKNOWN`); waiting on a review environment is never a failure; unreadable environment configuration is `UNKNOWN`. |
| CI-1003 | DONE | CI → deployment relationship (observed process links, no implied causality). | CI-1001 | `pipelineRunId` attached only from the provider-identified `task_id`, matching the G5 run-identity invariant; no invented correlation. |
| CI-1004 | DONE | Deployment history (enough for later rollback/outcome research). | CI-1001 | Bounded deployment/status acquisition with explicit page/count limits, deterministic ordering, start/completion from the status sequence, and per-source completeness reasons. |
| CI-1005 | DONE | Route `deployment` / `deployment_status` (and `workflow_run`/`check_suite` where used) webhooks to the adapter (provider-neutral after routing). | CI-1001 | Both events route to a provider-neutral `deployment` kind; the orchestrator acknowledges them derived-only (no D1 writes, no attention changes). `workflow_run`/`check_suite` carry no deployment identifier and are truthfully not routed. |

**G10 exit: DONE on the bounded shadow path.** Provider-neutral process semantics survive the bounded CD extension: deployments reuse the CI lifecycle/outcome vocabulary plus a separate approval state, so waiting for approval is never failure and `completed != successful` unless the provider and environment both say so. Exact-revision filtering, explicit bounds, and per-source completeness keep the history bounded and honest; CI→deployment links exist only where the provider identifies them. Deployment webhooks stop being silently ignored without touching the live path, and CI-710 derives a provenance-bearing `DEPLOYMENT_STATE` insight from the resulting observations. The boundary is recorded in [`CI_CD_DEPLOYMENT_EXTENSION_DECISION.md`](./CI_CD_DEPLOYMENT_EXTENSION_DECISION.md). Full verification: 54 test files / 430 tests, TypeScript, and diff checks. No ingestion, persistence-write, frontend, Check, attention, agent-action, or ML changes.
...
| 2026-09-02 | Completed the bounded G10 CD extension; unblocked and completed CI-710. | Deployment observations reuse the process vocabulary with a separate approval state (waiting for approval is not a failure); exact-revision filtering; bounded deployment/status history with completeness reasons; CI→deployment links only from the provider-identified `task_id`; `deployment`/`deployment_status` webhooks route to a provider-neutral kind handled derived-only (no D1 writes, no attention changes); `DEPLOYMENT_STATE` insights carry provenance, completeness, and `SUPPORTED`/`TENTATIVE`/`UNKNOWN` confidence. Boundary recorded in `CI_CD_DEPLOYMENT_EXTENSION_DECISION.md`. |
...
**Immediate execution order:** bounded G0–G10 **done** (CI-306 explicitly blocked/non-blocking) → next: plan re-review — re-attempt a CI-306 alternative (structured matrix coordinates / `needs` correlation) or define the next bounded extension. Do not begin with UI, agent integration, or historical ML.
